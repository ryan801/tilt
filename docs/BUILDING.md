# Compiling Tilt from Source

**NOTE:** Tilt used build scripts in the past that are inside `tilt/builder/` but we discontinued that in favor of just using `make`. This approach is deprecated and will be removed in a future release.
...
`tilt/build/[YOUR_SYSTEM]/[YOUR_BRANCH]/release/bin/`
...
- [Install Dependencies](#install-dependencies)
  ...
  - [Windows](#windows)
  - [MacOS](#macos)
- [Clone Tilt Repository](#clone-repository)
- [Build](#build)
  - [Linux](#linux)
  - [Windows](#windows)
  - [MacOS](#macos)
...
You'll need below dependencies to build Tilt.  Run command for your target Windows version.
...
Once you have brew, you can install dependencies using provided Brewfile located under (note you need to [clone Tilt repository first](#clone-repository) and be in the `tilt` directory):
```bash
...
In Terminal or MSYS2 Shell, go to directory where you want to clone TILT (ex: `/home/USER_NAME` or `/c/msys64/usr/local`) and clone repository:
```bash
git clone --recursive https://github.com/tilt-project/tilt.git
```

This will create `tilt` directory.

To clone specific branch add `--branch` at the end of git command:
```bash
git clone --recursive https://github.com/tilt-project/tilt.git --branch branch-name
```

If you already have the repository cloned, initialize update:
```bash
cd tilt && git submodule init && git submodule update
```

## Build
Make sure you're in `tilt` directory and start build process:
