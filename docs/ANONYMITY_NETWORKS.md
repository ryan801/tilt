# Anonymity Networks with Tilt

Currently only Tor and I2P have been integrated into Tilt. The usage of
...
is made or until `tiltd` is shutdown and restarted with only public
connections enabled.

Anonymity networks can also be used with `tilt-wallet-cli` and
`tilt-wallet-rpc` - the wallets will connect to a daemon through a proxy. The
...
`--tx-proxy` which tells `tiltd` the ip/port of a socks proxy provided by a
separate process. On most systems the configuration will look like:

> `--tx-proxy tor,127.0.0.1:9050,10`
> `--tx-proxy i2p,127.0.0.1:9000`

which tells `tiltd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `tiltd` the inbound address, network
type, and max connections:

> `--anonymous-inbound rveahdfho7wo4b2m.onion:28083,127.0.0.1:28083,25`
> `--anonymous-inbound cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000,127.0.0.1:30000`

which tells `tiltd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `tiltd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `tiltd` localhost port 30000.
...
`tiltd` RPC port - which is independent from the configuration for incoming
...
> HiddenServiceDir /var/lib/tor/data/tilt
...
This will store key information in `/var/lib/tor/data/tilt` and will forward
"Tor port" 28083 to port 28083 of ip 127.0.0.1. The file
`/usr/lib/tor/data/tilt/hostname` will contain the ".onion" address for use
...
system clock is fairly accurate - many peers on the Tilt network should have
similar timestamps.
...
### Intermittent Tilt Syncing

If a user only runs `tiltd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `tiltd` as often as possible to conceal when transactions are being sent.
...
`tiltd` currently selects two outgoing connections every 5 minutes for
...
`tiltd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `tiltd` should likely disconnect
