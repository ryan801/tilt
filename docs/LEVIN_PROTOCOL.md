...
used by Tilt. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Tilt's
...
> This document does not currently list all data being sent by the tilt
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about Tilt p2p traffic!


## Header
This header is sent for every Tilt p2p message.
...
Tilt when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Tilt specific
...
### Requests
Requests are the basis of the admin protocol for Tilt. The `Q` bit must be
